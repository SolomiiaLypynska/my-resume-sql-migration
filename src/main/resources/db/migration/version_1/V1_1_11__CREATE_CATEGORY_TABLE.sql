...
FROM ALL_OBJECTS
WHERE OBJECT_NAME = UPPER('CATEGORY')
    AND OBJECT_TYPE = UPPER('TABLE');

SELECT COUNT(1) INTO IS_SEQ_EXISTS
FROM ALL_OBJECTS
WHERE OBJECT_NAME = UPPER('SEQ_CATEGORY')
...
EXECUTE IMMEDIATE q'[
    CREATE TABLE CATEGORY (
        ...
        CONSTRAINT FK_CATEGORY_USER_ID FOREIGN KEY (USERS_ID) REFERENCES USERS(ID)
        ON DELETE CASCADE)
]';
END IF;
...
EXECUTE IMMEDIATE q'[
    CREATE SEQUENCE SEQ_CATEGORY MINVALUE 0 MAXVALUE 999999999999999999
